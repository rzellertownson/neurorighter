...
namespace SharanyaExperiments
{
    ...
    
    {
        ...
        public  override void  run()
        {
            //debug stuff
            //file header
            fs = new FileStream("twoelectrode.txt", FileMode.Create);
            ...
            try
            {
                ...
                while (!CLE.isCancelled & DateTime.Now < timeend)//wait for 1 day to pass, or for the experiment to be cancelled.
                {
                    ...
                    {
                        string response = "";
                        for (int i = 0; i < stimTimes.Length; i++)
                        {
                            response += stimTimes[i] + "ms ";
                        }
                        w.WriteLine(DateTime.Now.TimeOfDay.ToString() + ":appended at times " + response);
        ...
        public override void close()
        {
            try
            {
                ...
                w.WriteLine("two electrode pulse experiment stopped at " + DateTime.Now.TimeOfDay.ToString());
                w.Flush();
                w.Close();
                fs.Close();
                MessageBox.Show("two electrode pulse experiment stopped successfully");
            }
            catch (Exception e)
            {
