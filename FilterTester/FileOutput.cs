...
namespace NeuroRighter
{
    ...
    internal class FileOutput
    {
        ...
        internal virtual void writeHeader(int samplingRate){ }

        internal virtual void writeHeader(int numChannels, int samplingRate, int fileType, Task recordingTask, double preampgain)
        {
                        ...
                        if (_currentLocationRead[c] + length < BUFFER_LENGTH)
                        {
                            ...
                            if (_currentLocationWrite[c] > _currentLocationRead[c])
                            {
                                if (_currentLocationWrite[c] - _currentLocationRead[c] < length)
                                    System.Windows.Forms.MessageBox.Show("File Writer Buffer Overrun");
                            }
