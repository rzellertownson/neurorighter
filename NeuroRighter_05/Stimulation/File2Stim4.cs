...
namespace NeuroRighter
{
    class File2Stim4
    {
        ...
        {
            //open .olstim file
            StreamReader file = new StreamReader(stimfile);
            line = file.ReadLine(); // one read to get through header
            line = file.ReadLine(); // this read has the number of stimuli
            numstim = Convert.ToInt32(line); // find the number of stimuli specified in the file
            line = file.ReadLine(); // this read has the number samples in each stimulus
            ...
            int numberStimPerLoad = 500;
            int numFullLoads = (int)Math.Floor((double)numstim / (double)numberStimPerLoad);

            
            if (4*numberStimPerLoad >= numstim)
            {
            ...
            TimeVector = new int [numberStimPerLoad];
            ChannelVector = new int [numberStimPerLoad];
            WaveMatrix = new double [numstim, wavesize];

            // Load the first stimuli
            loadStimWithWave(file,numberStimPerLoad);
            ...
            // Loading loop
            int numLoadsCompleted = 1;
            while (numLoadsCompleted <= numFullLoads)//logic to determine if exit or not
            {

                //load next stims when appropriate
                if (stimbuff.stimuliInQueue() < numberStimPerLoad) //if there are less than numberStimPerLoad stimuli in the outer buffer
                    loadStimWithWave(file,numberStimPerLoad);
            ...
            TimeVector = new int [numstim - numFullLoads*numberStimPerLoad];
            ChannelVector = new int [numstim - numFullLoads*numberStimPerLoad];
            WaveMatrix = new double[numstim - numFullLoads*numberStimPerLoad, wavesize];
            loadStimWithWave(file,numstim - numFullLoads*numberStimPerLoad);
            stimbuff.append(TimeVector, ChannelVector, WaveMatrix); //add N more stimuli
        ...
        internal void loadStimWithWave(StreamReader olstimFile, int numStimToRead)
        {
            ...
            char delimiter = ' ';
            string[] splitWave;
            while ((line = olstimFile.ReadLine()) != null && j <= numStimToRead - 1)
            {
                // load stim time
