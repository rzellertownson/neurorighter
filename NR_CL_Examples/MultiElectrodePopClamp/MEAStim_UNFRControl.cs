...
namespace NR_CL_Examples
{
    ...
    public class MEAStim_UNFRControl : NRTask
    {
        ...
        // cl timing start
        double startTime;
        double[] fTargetList = { 3 }; // target firing rate (Hz)
        double clStartSec = 60; // seconds before tracking set point
        double clTrackSec = 300; //6*3600; // seconds of setpoint step

        // Optional stuff
        bool stopUpdate = false;
        double stopUpdateSec = 300;

        // algorithm parameters
        int[] stimElectrodes = { 21, 14, 53, 28, 9, 37, 44, 20, 12, 62 };
        ...
        protected override void Setup()
        {
            ...
            // Initialize target
            fTarget = fTargetList[targetIndex];
        }
        ...
        protected override void Loop(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // Update run time
                    double runTimeSec = NRStimSrv.StimOut.GetTime() / 1000 - startTime;

                    // Update the clock tick for repeated or non-repeated protocols
                    if (repPro)
                    {
                        double clockTickSecTmp = runTimeSec % proTime;
                        if (clockTickSecTmp < clockTickSec)
                        {
                            RefreshFeedbackVar();
                            UpdateTarget();
                        }
                        clockTickSec = clockTickSecTmp;
                    }
                    else
                    {
                        clockTickSec = runTimeSec;
                    }
                    ...
                    if (clockTickSec > clStartSec && clockTickSec <= proTime)
                    {
                        ...
                        //if (clockTickSec <= stopUpdateSec + clStartSec)
                        //{
                        // Cacluate feedback voltage
                        CreateFeedback();
        ...
        private void UpdateFineTuning()
        {
            ...
            for (int chan = 0; chan < stimElectrodes.Length; chan++)
            {
                double tFA = tuningFactor.Average();
                tuningFactorNorm[chan] = tuningFactor[chan] / tFA;

                // Update stimulation voltage
                channelVoltage[chan] = stimVoltage * tuningFactorNorm[chan]; // update actual stim voltage for this channel

                //Make sure that channelVoltage does not exceed maxima
        ...
        private void EncodeDigInfo()
        {
            ...
            if (nextStimSample <= (nextAvailableSample + (ulong)NRStimSrv.GetBuffSize()))
            {
                ...
                toAppendDig.Add(new DigitalOutEvent(nextStimSample, Convert.ToUInt32(100 * currentTarget)));
            }
